...
def supports_oft(
    model: type[object] | object,
) -> TypeIs[type[SupportsOFT]] | TypeIs[SupportsOFT]:
    ...
    if not result:
        ...
        missing_attrs = tuple(attr for attr in oft_attrs if not hasattr(model, attr))
        ...
        if getattr(model, "supports_oft", False):
            ...
            if not missing_attrs:
                logger.warning(
                    ...
                    "but does not set `supports_oft=True`.",
                    model,
                )
...
def _supports_oft(model: type[object] | object) -> bool:
    if isinstance(model, type):
