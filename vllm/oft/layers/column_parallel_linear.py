...
def _mcp_apply(x, bias, layer: "ColumnParallelLinearWithOFT"):
    ...
    oft_output: Optional[torch.Tensor] = layer.punica_wrapper.add_expand(
        output,
        buffers,
...
class ColumnParallelLinearWithOFT(BaseLinearLayerWithOFT):
    ...
    def slice_oft_r(self, oft_r: torch.Tensor) -> torch.Tensor:
        return oft_r
...
class MergedColumnParallelLinearWithOFT(ColumnParallelLinearWithOFT):
    ...
    def create_lora_weights(
        ...
        lora_a_output_size_per_partition = (
            oft_config.max_lora_rank if not oft_config.fully_sharded_loras
    ...
    def slice_oft_r(
        self, oft_r: list[Union[torch.Tensor, None]]
...
class QKVParallelLinearWithOFT(ColumnParallelLinearWithOFT):
    ...
    def can_replace_layer(cls, source_layer: nn.Module,
                          oft_config: OFTConfig, packed_modules_list: list,
...
class MergedQKVParallelLinearWithOFT(MergedColumnParallelLinearWithOFT):
    ...
    def create_oft_weights(
        ...
        """
        super().create_lora_weights(max_ofts, oft_config, model_config)
...
class MergedColumnParallelLinearWithShardedOFT(
